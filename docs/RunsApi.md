...
# **get_run_steps**
> StepList get_run_steps(owner, name, run_id, status=status, step_id=step_id, since_generation=since_generation, page=page, per_page=per_page)
...
per_page = 25 # int | Number of items per page (optional) (default to 25)
    ...
    try:
        # Query the steps of a run
        api_response = api_instance.get_run_steps(owner, name, run_id, status=status, step_id=step_id, since_generation=since_generation, page=page, per_page=per_page)
        pprint(api_response)
    except ApiException as e:
